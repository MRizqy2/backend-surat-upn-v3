...
const putHeaderDaftarSurat = async (req, res) => {
  try {
    let {
      user_id,
      judul,
    ...
    let dataHeader = await HEADER_DAFTAR_SURAT.findOne({
      where: { id: user_id },
    });

    dataHeader = await HEADER_DAFTAR_SURAT.update(
      {
    ...
    if (dataHeader) {
      dataHeader = await HEADER_DAFTAR_SURAT.findOne({
        where: { id: user_id },
      });
    }
    res.status(StatusCodes.OK).json({
      message: "Header daftar surat updated successfully",
      header_daftar_surat: dataHeader,
    });
  } catch (error) {
...
module.exports = { putHeaderDaftarSurat, app };
